...
public class ClientControllerTest {
	...
	public void setUp() throws MalformedHandlerException, LoanException,
		...
		bank = new Bank(new TransactionManager(), new BankHandler("1111"));
		office = new Office(new OfficeHandler("1234"), bank);
		account = new Account(office, bank, "1234456789");
		client = new Client(new DNIHandler("71560136Y"));
		loan = new Loan(new LoanIdentificationNumberCode("MG", "ES"), 10000,
				0.05, PaymentPeriod.MONTHLY, 12, account, client);
		client.setName("Carlos");
		client.setSurname("Mayo");
		account.addTitular(client);
		client.addLoan(loan);
	}
